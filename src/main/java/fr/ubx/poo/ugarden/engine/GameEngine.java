    ...
    public final class GameEngine {
        ...
        private void initialize() {
            ...
            for(int i = 0; i < game.getSizeHornets(); i++){
                sprites.add(new SpriteHornet(layer, game.getHornet(i)));
            }
        }
        ...
        private void checkCollision() {
            ...
            for(int i = 0; i < game.getSizeHornets(); i++){
                Position hornetPosition = game.getHornet(i).getPosition();
                boolean test1 = Direction.DOWN.nextPosition(gardener.getPosition()) == hornetPosition;
                boolean test2 = Direction.UP.nextPosition(gardener.getPosition()) == hornetPosition;
                boolean test3 = Direction.LEFT.nextPosition(gardener.getPosition()) == hornetPosition;
                boolean test4 = Direction.RIGHT.nextPosition(gardener.getPosition()) == hornetPosition;
                boolean test5 = gardener.getPosition() == hornetPosition;
                if(test1 || test2 || test3 || test4 || test5) {
                    game.getHornet(i).sting(gardener);
                }
            }
        }
        ...
        private void updateSprites(long now) {
            for(int i = 0; i < game.getSizeHornets(); i++){
                if(!game.getHornet(i).isSpriteUp()){
                    game.getHornet(i).setSpriteUp();
                    sprites.add(new SpriteHornet(layer, game.getHornet(i)));
                }
            }
        }
